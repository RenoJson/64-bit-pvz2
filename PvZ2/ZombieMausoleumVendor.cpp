...
void MausoleumVendorLoopWalk(ZombieMausoleumVendor* zombie)
{
    ...
    int zombieRow = static_cast<int>((zY - 160.0f) / 76.0f);
    ...
    GetEntitiesInRectPixel(&entities, 63, &attackRect, zombieRow, zombieRow);
    ...
    for (BoardEntity* ptr : entities) {
        ...
        if (ptr->IsType(GridItemMausoleumLawnPath::StaticGetType()))
        {
            ...
            GridItem* pathTile = static_cast<GridItem*>(ptr);
            ...
            auto* zombieProps = reinterpret_cast<ZombiePropertySheet*>(zombie->m_propertySheet.Get());
            float baseSpeed = zombieProps->Speed;
