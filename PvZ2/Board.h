...
public:
	...
	void* m_levelNameTextWidgetPtr;
	float m_backdropCameraRenderScale;
	float m_backdropResolutionX;
	float m_backdropResolutionY;
	Sexy::Rect m_backdropCameraRenderRect;
	char m_pad9[52];
	std::vector<GameSubsystem*> m_subsystems;
...
static_assert(offsetof(Board, m_backdropCameraRenderScale) == 1120);
static_assert(offsetof(Board, m_backdropResolutionX) == 1124);
static_assert(offsetof(Board, m_backdropResolutionY) == 1128);
...
inline static uint64_t GetSexyApp() {
	return *(uint64_t*)getActualOffset(0x25A4618);
}

inline static uint64_t GetLawnApp() {
